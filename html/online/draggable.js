...
class Draggable {
    ...
    StartDragging(event) {
        ...
        setTimeout(()=>{
            if(!this.down)return;
            this.lastElem=event.srcElement;
            this.Update();
            this.interval = setInterval(()=>{
                if(moved){
                    moved = false;
                    if(up_down){
                        this.isX&&this.CompareX(this.currentList,this.priorityx,x)||this.Compare(this.currentList,this.priorityy,y);
                    } else {
                        clearInterval(this.interval);
                        this.down=false;
                        return;
    ...
    Compare(list, priority, current) {
        const priorityIndex = list.indexOf(priority);
        if(priorityIndex === -1)return;
        const prevElement = this.currentElem[list[priorityIndex - 1]];
        const nextElement = this.currentElem[list[priorityIndex + 1]];
        if (current < priority && prevElement && current <= prevElement.getBoundingClientRect().y) {
            const pr = this.currentElem[priority];
            prevElement.parentNode.insertBefore(pr, prevElement);
            this.Update();
        } else if (current > priority && nextElement && current >= nextElement.getBoundingClientRect().y) {
            const pr = this.currentElem[priority];
            nextElement.parentNode.insertBefore(pr, nextElement.nextSibling);
            this.Update();
        };
    };
    CompareX(list, priority, current) {
